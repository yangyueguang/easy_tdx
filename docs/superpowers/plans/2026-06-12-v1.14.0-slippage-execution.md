...
class OrderSimulator:
    # ... existing fields ...
    slippage: float = 0.0
    slippage_model: SlippageModel = None  # NEW
...
class ExecutionModel(ABC):
    ...
    def _calc_slippage(
        self, size: float, price: float, is_sell: bool,
        slippage_model: SlippageModel, df: pd.DataFrame,
    ) -> float:
        """计算滑点。"""
...
def __init__(
    ...
    slippage_model: SlippageModel = None,     # NEW
    execution_model: ExecutionModel = None,     # NEW
):
...
def _find_bar_index(self, df: pd.DataFrame, datetime_val: int) -> int:
